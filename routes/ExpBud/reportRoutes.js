...
router.post("/", async (req, res) => {
  try {
    const { totalSpent, totalBudget, percentUsed, categories } = req.body;
    ...
    if (
      ...
      return res.status(400).json({ message: "Dữ liệu không hợp lệ hoặc thiếu" });
    }
...
router.get("/latest", async (req, res) => {
  try {
    ...
    if (!latestReport) {
      return res.status(404).json({ message: "Không tìm thấy report nào" });
    }
